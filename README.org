...
*** ORG ROAM
=Org Roam= is used to control my digital brain. The brain is located in my OneDrive Folder, but the DB lives outside to allow sharing of the documents accross multiple devices. When switching =org-roam-build-cache= should be called to regenreate the Database.
